...
#define EMA_ALPHA 0.5 // EMA weight of new sample (range: 0 to 1). Setting EMA to 1 effectively disables EMA filter.

unsigned long last_sampling_time; // unit: msec
float prev_distance = DIST_MAX; // distance last-measured
float ema_distance; // exponential moving average distance
...
void loop() {
    ...
    float raw_distance = measure_from_uss(PIN_TRIG, PIN_ECHO);
    if (raw_distance == 0.0 || DIST_MAX < raw_distance || raw_distance < DIST_MIN) raw_distance = prev_distance;
    ema_distance = EMA_ALPHA * raw_distance + (1.0 - EMA_ALPHA) * prev_distance;
    prev_distance = ema_distance;

    {
        float distance_from_target = ema_distance;
