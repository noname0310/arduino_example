...
void loop() {
  ...
  dist_ema = _EMA_ALPHA * dist_raw + (1.0 - _EMA_ALPHA) * dist_prev;
  dist_prev = dist_ema;

  // adjust servo position according to the USS read value

  float clamped_dist = constrain(dist_ema, _DIST_MIN, _DIST_MAX);
  int servo_pos = map(clamped_dist, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
